...
// Generated 26-Feb-2022 15:53:26
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"delay_brake\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(delay_brake_M, (NULL));
        delay_brake_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(delay_brake_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(delay_brake_M)
                == (NULL)) || rtmGetStopRequested(delay_brake_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(delay_brake_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
